...
int main() {
    ...
    // Вывод матрицы городов
    std::cout << "\nМатрица городов:\n";
    for (int i = 0; i < numCities; ++i) {
        for (int j = 0; j < numCities; ++j) {
            std::cout << std::setw(8) << std::fixed << std::setprecision(2) << cities[i][j] << " ";
        }
        std::cout << "\n";
    }
    ...
    auto get_dist = [](const std::vector<int>& travel, const std::vector<std::vector<double>>& cities) -> double {
        double dist = 0;
    ...
    while (!notincludet.empty()) {
        ...
        for (int i : travel) {
            auto min_it = std::min_element(notincludet.begin(), notincludet.end(), [&](int a, int b) {
                return cities[i - 1][a - 1] < cities[i - 1][b - 1];
                });
    ...
    double dist = get_dist(travel, cities);
    // Добавляем расстояние от последнего города до начального
